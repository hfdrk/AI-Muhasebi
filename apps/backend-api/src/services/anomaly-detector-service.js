...
exports.anomalyDetectorService = exports.AnomalyDetectorService = void 0;
const prisma_1 = require("../lib/prisma");
class AnomalyDetectorService {
    ANOMALY_THRESHOLD_FACTOR = 2.5; // Flag if value > mean * 2.5
    /**
     * Detect anomalies for a client company
     */
    async detectAnomalies(tenantId, clientCompanyId) {
        ...
        for (const [accountCode, latestAmount] of latestMonthData.entries()) {
            ...
            if (latestAmount > threshold) {
                const account = await prisma_1.prisma.ledgerAccount.findFirst({
                    where: {
                        tenantId,
                        code: accountCode,
                    },
     ...
     */
    groupByMonthAndAccount(transactions) {
    ...
    calculateAverageTransactionAmount(transactions) {
        ...
        const total = transactions.reduce((sum, t) => {
            const transactionTotal = t.lines.reduce((lineSum, line) => {
     ...
     */
    getCommonAccounts(historicalMonths, monthlyData) {
     ...
     */
    determineAnomalySeverity(value, threshold) {
...
exports.anomalyDetectorService = new AnomalyDetectorService();
