...
class ReportingService {
     ...
     */
    async validateClientCompany(tenantId, clientCompanyId) {
        const company = await prisma_1.prisma.clientCompany.findFirst({
            where: {
                id: clientCompanyId,
                tenantId,
            },
        ...
        if (!company) {
            throw new shared_utils_1.NotFoundError("Müşteri şirketi bulunamadı.");
        }
    }
    /**
     ...
     * ```
     */
    async generateCompanyFinancialSummary(tenantId, clientCompanyId, filters) {
     ...
     * @throws NotFoundError if client company is not found or doesn't belong to tenant
     */
    async generateCompanyRiskSummary(tenantId, clientCompanyId, filters) {
        await this.validateClientCompany(tenantId, clientCompanyId);
        const startDate = new Date(filters.start_date);
        const endDate = new Date(filters.end_date);
        // Get latest risk score for the company
        const latestRiskScore = await prisma_1.prisma.clientCompanyRiskScore.findFirst({
            where: {
                tenantId,
                clientCompanyId,
            },
        ...
        const documentsWithRiskScores = await prisma_1.prisma.document.findMany({
            where: {
                tenantId,
                clientCompanyId,
                createdAt: {
        ...
        const highRiskDocuments = documentsWithRiskScores.filter((doc) => doc.riskScore && doc.riskScore.severity === "high");
        // Query open risk alerts grouped by severity
        const openAlerts = await prisma_1.prisma.riskAlert.findMany({
            where: {
                tenantId,
                clientCompanyId,
                status: "open",
     ...
     */
    async generateTenantPortfolioReport(tenantId, filters) {
        const startDate = new Date(filters.start_date);
        const endDate = new Date(filters.end_date);
        ...
        for (const company of companies) {
            ...
            const highRiskInvoiceCount = await prisma_1.prisma.invoice.count({
                where: {
                    tenantId,
     ...
     */
    async generateDocumentActivityReport(tenantId, clientCompanyId, filters) {
        if (clientCompanyId) {
            await this.validateClientCompany(tenantId, clientCompanyId);
        }
        const startDate = new Date(filters.start_date);
