...
exports.riskAlertService = exports.RiskAlertService = void 0;
const prisma_1 = require("../lib/prisma");
const shared_utils_1 = require("@repo/shared-utils");
const notification_service_1 = require("./notification-service");
const email_service_1 = require("./email-service");
class RiskAlertService {
    mapToRiskAlert(alert) {
        return {
            id: alert.id,
    ...
    async createAlert(input) {
        ...
        const existing = await prisma_1.prisma.riskAlert.findFirst({
            where: {
        ...
        if (existing) {
            ...
            const updated = await prisma_1.prisma.riskAlert.update({
        ...
        const alert = await prisma_1.prisma.riskAlert.create({
            data: {
                tenantId: input.tenantId,
                clientCompanyId: input.clientCompanyId ?? null,
                documentId: input.documentId ?? null,
        ...
        try {
            ...
            try {
                ...
                if (recipientEmails.length > 0) {
                    await email_service_1.emailService.sendNotificationEmail(recipientEmails, "RISK_ALERT", notification.title, notification.message);
                }
            }
            catch (emailError) {
     ...
     */
    async listAlerts(tenantId, filters = {}) {
        ...
        const [data, total] = await Promise.all([
            prisma_1.prisma.riskAlert.findMany({
                where,
                skip,
                take: pageSize,
            ...
            prisma_1.prisma.riskAlert.count({ where }),
        ]);
        return {
     ...
     * Update alert status
     */
    async updateAlertStatus(tenantId, alertId, status, userId) {
        const alert = await prisma_1.prisma.riskAlert.findUnique({
            where: { id: alertId },
        });
        if (!alert || alert.tenantId !== tenantId) {
            throw new shared_utils_1.NotFoundError("Uyarı bulunamadı.");
        }
        const updateData = {
            status,
            updatedAt: new Date(),
...
exports.RiskAlertService = RiskAlertService;
exports.riskAlertService = new RiskAlertService();
//# sourceMappingURL=risk-alert-service.js.map