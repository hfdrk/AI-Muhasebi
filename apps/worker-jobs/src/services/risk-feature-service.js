...
exports.riskFeatureService = exports.RiskFeatureService = void 0;
...
class RiskFeatureService {
     ...
     */
    async generateRiskFeatures(parsedData, documentId, tenantId) {
        const features = {};
        const flags = [];
        // Process based on document type
        if (parsedData.documentType === "invoice") {
            const invoiceFields = parsedData.fields;
            ...
            const statementFields = parsedData.fields;
    ...
    async checkInvoiceRisks(fields, tenantId, documentId, features, flags) {
    ...
    checkBankStatementRisks(fields, features, flags) {
        ...
        if (fields.startingBalance !== null &&
        ...
        if (fields.transactions && fields.transactions.length > 0) {
            const highTransactions = fields.transactions.filter((t) => t.amount && Math.abs(t.amount) > 100000);
    ...
    async checkDuplicateInvoiceNumber(tenantId, currentDocumentId, invoiceNumber, features, flags) {
        try {
            ...
            const existing = await prisma_1.prisma.documentParsedData.findMany({
                where: {
                    tenantId,
...
exports.RiskFeatureService = RiskFeatureService;
exports.riskFeatureService = new RiskFeatureService();
//# sourceMappingURL=risk-feature-service.js.map